							...
							<div className='group grid grid-cols-4 items-center hover:bg-regal-blue-hover  bg-regal-blue rounded-full py-4 px-12'>
								<div className='w-fit h-fit py-3 px-6 hover:bg-green-hover hover:rounded-full flex flex-col items-center'>
									{inputShow ? (
										<input
											...
											className='bg-transparent text-white text-xl px-2 rounded'
										/>
									) : (
								...
								{showResponsible ? (
									<input
										...
										className='bg-transparent text-white text-sm px-2 rounded w-full'
									/>
								) : (
									<div
								...
								{selectedType ? (
									<select
										...
										className='bg-blue-light text-black text-sm px-2 py-1 rounded-full'
									>
										<option>Входящий</option>
										<option>Исходящий</option>
										<option>Пропущенный</option>
									</select>
								) : (
									<div
										onClick={() => setSelectedType(true)}
										className='width-calls h-fit flex justify-center items-center gap-2 justify-self-end bg-blue-light hover:bg-blue-light-hover text-[14px] text-black py-1 px-4 rounded-full'
									>
										{call.type}
										<IoIosArrowDown />
									</div>
								...
								{selectedPriority ? (
									...
									<div
										...
										className='w-fit h-fit flex items-center justify-self-end gap-2 bg-white-20 hover:bg-white-20-hover text-[14px] text-white py-1 px-4 rounded-full'
									>
										{call.priority}
										<IoIosArrowDown />
									</div>
